...
def chow_liu(observations):
    ...
    for i in range(num_vars):
        for j in range(i+1, num_vars):
            empirical_mutual_infos.append(
                    (compute_empirical_mutual_info_nats(
                         observations[:, i], observations[:, j]),
                     (i, j)))
    sorted_empirical_mutual_infos = sorted(empirical_mutual_infos,
                                           reverse=True)
    ...
    for empirical_mutual_info, (i, j) in sorted_empirical_mutual_infos:
        ...
        if union_find.find(i) != union_find.find(j):
            ...
            best_tree.add((i, j))
            # also: tell the union-find data structure that i and j are now in
...
def compute_empirical_conditional_distribution(var1_values, var2_values):
    ...
    for x2 in p_var2:
        for x1 in var1_values_unique:
            if (x1, x2) in p_var1var2 and p_var1var2[(x1, x2)] > 0:
                conditional_distributions[x2][x1] = \
                    p_var1var2[(x1, x2)] / p_var2[x2]
            ...
            if not visited[neighbor]:
                edge_potentials[(node, neighbor)] = \
                    compute_empirical_conditional_distribution(
                            ...
                            observations[:, node])
                edge_potential_transposed = {}
                for x2 in edge_potentials[(node, neighbor)]:
                    for x1 in edge_potentials[(node, neighbor)][x2]:
                        if x1 not in edge_potential_transposed:
                            edge_potential_transposed[x1] = \
                                {x2: edge_potentials[(node, neighbor)][x2][x1]}
                        else:
                            edge_potential_transposed[x1][x2] = \
                                edge_potentials[(node, neighbor)][x2][x1]
        ...
        for j in neighbors:
            ...
            for x_i in node_potentials[i]:
                ...
                for x_j in node_potentials[j]:
                    if x_j in edge_potentials[(j, i)] and \
                            x_i in edge_potentials[(j, i)][x_j]:
                        product = \
                            node_potentials[j][x_j] * \
                            edge_potentials[(j, i)][x_j][x_i]
